...
class RichJsValueSpec
  ...
  "set" - {
    ...
    "must set a nested value on a JsArray" in {
      ...
      value.set(path, JsString("bar")).asOpt.value mustEqual Json.arr(
        Json.arr("bar")
      )
    }
    ...
    "must return an error when trying to set a key on a non-JsObject" in {
      ...
      value.set(path, JsString("bar")) mustEqual JsError(
        s"cannot set a key on $value"
      )
    }
    ...
    "must return an error when trying to set an index on a non-JsArray" in {
      ...
      value.set(path, JsString("bar")) mustEqual JsError(
        s"cannot set an index on $value"
      )
    }
    ...
    "must remove a value given a keyPathNode and return the new object" in {
      ...
      forAll(gen) { case (keys, values, keyToRemove, valueToRemove) =>
        ...
        val testObject: JsObject =
          initialObj + (keyToRemove -> Json.toJson(valueToRemove))
  ...
  "remove a value when there are nested arrays" in {
    ...
    val input = Json.obj(
      "key" -> JsArray(
        Seq(JsArray(Seq(Json.toJson(1), Json.toJson(2))), Json.toJson(2))
      ),
      "key2" -> JsArray(Seq(Json.toJson(1), Json.toJson(2)))
    )
